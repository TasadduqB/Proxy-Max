...
function flushLogLines() {
  ...
  try { rotateLogs(); fs.appendFile(LOG_FILE, lines.join('\n') + '\n', 'utf8', e => { if (e) console.error('[proxy] [log-write-error]', e.message); }); }
  catch (e) { console.error('[proxy] [log-write-error]', e.message); }
}
function writeLogLine(entry) {
  try {
    const line = logLineFromEntry(entry);
    if (LOG_WRITE_MODE === 'sync') { rotateLogs(); fs.appendFileSync(LOG_FILE, line + '\n', 'utf8'); return; }
...
function readLogEntries(limit = 200) {
  ...
  try {
    ...
    return { logs: [], total: 0, limit: max, logFile: LOG_FILE, logFiles: [] };
  }
}
...
function handleLogsFileGet(req, res) {
  ...
  send(res, 200, readLogEntries(limit));
}
...
async function handleDiagnosticsGet(req, res) {
  try {
    ...
    const [{ logs, total, logFile, logFiles }, summary, modelRows, costRows, tokenRows, overview] = await Promise.all([
      Promise.resolve(readLogEntries(limit)),
